<template lang="html">
  <div class="container favorites">
    <div class="card-panel" v-if="getFavoritesStoryNum === 0">
     <h5 class="flow-text center-align">还没有收藏文章.</h5>
    </div>
    <div v-else>
      <router-link tag="a" class="col l6 m12 s12"
        :to="{name: 'story', params: {id: story.id }}" :key="story.id" v-for="story of getFavorites">
      ...
      <button class="waves-effect waves-light btn clear-all" @click="clearAllStories">清除全部收藏</button>
    </div>
  </div>
</template>
